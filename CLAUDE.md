...
## Working Principles
- **KISS:** выбирай простое решение. Один `rsync` лучше кастомного скрипта. Если решение требует длинного объяснения — оно слишком сложное.
- **YAGNI:** не создавай файлы, скрипты, абстракции «на будущее». Не нужно сейчас — не делай.
- **Чистота:** удаляй устаревшие TODO, пустые папки, битые ссылки, неиспользуемые файлы. Цифровая гигиена = ясность.
- **Plan B:** при рекомендации инструмента/сервиса — всегда указывай альтернативу, которая встаёт на замену без переделки.
- **Чеклист перед усложнением:**
  1. Это нужно ПРЯМО СЕЙЧАС?
  2. Какую реальную проблему решает?
  3. Можно решить проще?
  4. Добавляет ли новую зависимость?
  5. Я пойму это через 3 месяца?
