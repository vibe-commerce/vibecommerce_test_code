...
# --- Правила ABCDX-тестовых кейсов ---
# X: новинки — SKU появляется в продаже только с указанной недели (wi)
novelty_rules = {8: 4, 45: 6, 49: 8}  # W5, W7, W9

# D: убыточный SKU 18 — глубокий слив остатков в W9-W12
#   - скидка 50% к базовой цене (ниже себестоимости после комиссии/логистики)
#   - удвоенный ДРР (агрессивная реклама распродажи)
LOSS_SKU = 18
LOSS_WEEKS = {8, 9, 10, 11}  # W9-W12
LOSS_DISCOUNT = 0.50
LOSS_DRR_BOOST = 2.0
    ...
    for p in products:
        ...
        for platform in ["WB", "Ozon"]:
            ...
            if (pid, platform, wi) in oos_rules:
                qty = 0
            elif pid in novelty_rules and wi < novelty_rules[pid]:
                qty = 0
            else:
            ...
            if pid == LOSS_SKU and wi in LOSS_WEEKS:
                price = round(price * LOSS_DISCOUNT)
...
# ============================================================
print(f"\n{'='*70}")
print("  ABCDX-АНАЛИЗ ТОВАРОВ")
print("  Период: последний месяц (12 янв — 2 фев 2026)")
print(f"{'='*70}\n")
